...
namespace IVU.Http.Headers.Managed
{
    ...
    {
        private readonly ILog logger = LogProvider.For<HttpConnectionPool>();
        ...
        {
            this.logger.Trace("Created HTTP connection pool with max {0} connections", maxConnections);
            ...
            lock (SyncObj)
            {
                ...
                while (list.Count > 0)
                {
                    ...
                    if (cachedConnection.IsUsable())
                    {
                        ...
                        this.logger.Trace("HTTP connection pool: Found a cached connection");
                ...
                {
                    this.logger.Trace("HTTP connection pool: Creating a new connection");
                ...
                else
                {
                    ...
                    this.logger.Trace("HTTP connection pool: Wait for a connection to be returned to the pool");
        ...
        {
            this.logger.Trace("HTTP connection pool: Return connection to the pool");
        ...
        {
#if !DEBUG
            this.logger.Trace("HTTP connection pool: Clean cache");
#endif
            ...
            try
            {
                ...
                if (freeIndex < list.Count)
                {
                    ...
                    if (list.Count - freeIndex > 0)
                    {
                        this.logger.Trace("HTTP connection pool: Clean cache: remove {0} items", list.Count - freeIndex);
                    }
        ...
        private sealed class ConnectionWaiter<TState> : ConnectionWaiter
        {
            ...
            public override ValueTask<HttpConnection> CreateConnectionAsync()
            {
                ...
                {
                    return new ValueTask<HttpConnection>(System.Threading.Tasks.Task.FromException<HttpConnection>(e));
                }
            }
        }
