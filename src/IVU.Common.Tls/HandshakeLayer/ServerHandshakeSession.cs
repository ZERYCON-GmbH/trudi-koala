...
namespace IVU.Common.Tls.HandshakeLayer
{
    ...
    using Protocol;
    using Plugin;
    using Plugin.CipherSuitePluginInterface;

    using CipherSuite = Plugin.CipherSuitePluginInterface.CipherSuite;

    public class ServerHandshakeSession : HandshakeSession
    {
        private static readonly ILog Log = LogProvider.For<ServerHandshakeSession>();
